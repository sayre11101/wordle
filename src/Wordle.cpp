...
void Wordle::printmenu() const
{
	...
	cout << "7) list guesses so far" << endl;
	cout << "8) print list of words used to date" << endl;
	cout << "0) quit" << endl;
}
...
void Wordle::process_menu_item_1()
{
	...
	vector<Wordstr> &word_list =
		(m_possibilities.size() == 0) ? valid_wordle_answers : m_possibilities.back();
	for (auto wordstr : word_list)
	{
		Word word(wordstr);
		if (word.test(guess))
		{
			if (words_so_far.count(wordstr) == 0)
			{
...
void Wordle::run()
{
	...
	{
		ifstream in = ifstream("words-so-far.txt");
		string s;
		while (in >> s)
		{
			Wordle::words_so_far.insert(Wordstr(s));
		}
	}
	bool running = true;
	while (running)
	{
		...
		case 5:
			cout << endl
				 << "enter letters for search: " << flush;
			cin >> letters;
